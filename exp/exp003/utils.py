...
def get_dataset(config):
    num_extra_site = config['feature']['num_extra_site']
    num_feats = config['feature']['num_feats']
    bssid_feats = [f'bssid_{i}' for i in range(num_feats)]
    rssi_feats = [f'rssi_{i}' for i in range(num_feats)]
    ssid_feats = [f'ssid_{i}' for i in range(num_feats)]

    ###### 24site以外も追加
    df_test = pd.read_csv('./../../data_ignore/nb/028/test_wifi_num_80.csv')
    list_test_site = sorted(df_test.site_id.unique())
    list_path = sorted(glob('./../../data_ignore/nb/028/*train*.csv'))
    list_path_train = []
    for path in list_path:
        for site in list_test_site:
            if site in path:
                list_path_train.append(path)
    list_path_train_extra = sorted(list(set(list_path) - set(list_path_train)))

    random.seed(config['feature']['seed'])
    list_path_train_extra = list(random.sample(list_path_train_extra, len(list_path_train_extra)))
    list_path_train_extra = list_path_train_extra[:num_extra_site]
    list_path_train_merge = list_path_train + list_path_train_extra
    list_df_train = []
    for path in list_path_train_merge:
        df = pd.read_csv(path, engine='python')
        list_df_train.append(df)
    df_train = pd.concat(list_df_train, axis=0)
    extra_site = list(set(df_train.site_id.values) - set(df_test.site_id.values))
    ...
    for i in range(num_feats):
        wifi_bssids.extend(df_train.iloc[:, num_feats+i].values.tolist())
    ...
    wifi_bssids_test = []
    for i in range(num_feats):
        wifi_bssids_test.extend(df_test.iloc[:, num_feats+i].values.tolist())
    ...
    df_train = df_train.drop(ssid_feats, axis=1)
    df_test = df_test.drop(ssid_feats, axis=1)
    return df_train, df_test, sub, bssid_feats, rssi_feats, wifi_bssids_size, extra_site
