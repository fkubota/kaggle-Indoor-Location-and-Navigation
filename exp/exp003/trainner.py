...
def train_cv(config, run_name):
    ...
    # ----------------
    df_train, df_test, sub, bssid_feats, rssi_feats, wifi_bssids_size, extra_site = U.get_dataset(config)
    if debug:
        df_train = df_train.iloc[::50, :].reset_index(drop=True)
    ...
    for i_fold, (trn_idx, val_idx) in enumerate(gkf.split(df_train.loc[:, 'path_id'], groups=df_train.loc[:, 'path_id'])):
